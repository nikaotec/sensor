...
const SystemInfoPanel: React.FC<SystemInfoPanelProps> = ({
    ...
    return (
        <div className="rounded-2xl border border-[#2A2E24] bg-[#1A1D17] p-6 shadow-lg">
            <h3 className="text-xs font-medium text-slate-400 uppercase mb-4 tracking-wider font-heading">
            ...
            <div className="space-y-4">
                ...
                <div className="flex justify-between text-sm">
                    ...
                    <span className="font-mono text-white font-medium">
                        {device?.telemetry?.uptime
                            ? `${Math.floor(device.telemetry.uptime / 3600)}h ${Math.floor((device.telemetry.uptime % 3600) / 60)}m`
                            : '--'}
                    </span>
                </div>
                ...
                <div className="pt-5 mt-5 border-t border-[#2A2E24] flex items-center justify-between">
                    ...
                    <button
                        ...
                        className={`relative inline-flex h-6 w-12 rounded-full border-2 transition-colors ${remoteSync ? 'bg-primary border-transparent' : 'bg-[#0F110D] border-[#2A2E24]'}`}
                    >
