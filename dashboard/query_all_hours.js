...
async function test() {
    const { data: data8, error: error8 } = await supabase.from('telemetry').select('data_registro, hora_registro').gte('hora_registro', '08:00:00').lt('hora_registro', '09:00:00').limit(5);
    const { data: data16, error: error16 } = await supabase.from('telemetry').select('data_registro, hora_registro').gte('hora_registro', '16:00:00').lt('hora_registro', '17:00:00').limit(5);
