...
const tsxFiles = ['Alerts.tsx', 'DeviceDetails.tsx', 'DeviceList.tsx', 'Login.tsx', 'Reports.tsx', 'Settings.tsx', 'Sidebar.tsx'];
...
for (const file of tsxFiles) {
    ...
    if (file === 'Login.tsx') {
        content = content.replace(/currentTenant\.name/g, 'currentTenant?.name');
        content = content.replace(/value=\{currentTenant\.id\}/g, 'value={currentTenant?.id || ""}');
        fs.writeFileSync(fullPath, content);
        continue;
    }
    ...
    if (match) {
        if (!content.includes('if (!currentTenant) return')) {
            ...
            const newContent = content.slice(0, index) + '\n    if (!currentTenant) return <div className="p-8 text-white">Carregando dados...</div>;' + content.slice(index);
