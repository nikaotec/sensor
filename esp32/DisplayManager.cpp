...
void DisplayManager::showMessage(String msg, int duracaoMs) {
    mensagemRodape = msg;
    currentMessage = msg; // Store needed for paging logic if needed, though we use mensagemRodape
    currentMsgPage = 0;
...
void DisplayManager::update(float temp, float max, float min, float voltage, bool wifiConnected, bool manual, bool relay, bool alarm) {
    ...
    if (now - lastDisplayUpdate > 250) {
        ...
        // Temperatura Grande
        char tB[10]; dtostrf(temp, 4, 1, tB);
        ...
        display.setFont(u8g2_font_6x12_tf);
        dtostrf(max, 4, 1, mB);
        display.drawStr(75, 26, "Max"); display.drawStr(98, 26, mB);
        dtostrf(min, 4, 1, mB);
        display.drawStr(75, 42, "Min"); display.drawStr(98, 42, mB);
        ...
        String pageText = mensagemRodape.substring(startIndex, startIndex + pageSize);
