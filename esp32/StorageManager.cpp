...
void StorageManager::load() {
    ...
    if (isnan(data.voltCalFactor) || data.voltCalFactor < 10.0 || data.voltCalFactor > 1000.0) {
        data.voltCalFactor = VOLTAGE_CALIBRATION_DEFAULT;
        EEPROM.put(ADDR_VOLT_CAL, data.voltCalFactor);
        EEPROM.commit();
    }
    
    if (isnan(data.alarmMax)) data.alarmMax = 8.0;
    if (isnan(data.alarmMin)) data.alarmMin = 2.5;
    if (isnan(data.voltMax)) data.voltMax = 245.0;
    if (isnan(data.voltMin)) data.voltMin = 190.0;
    
    if (isnan(data.tempMaxRec) || data.tempMaxRec > 80.0) data.tempMaxRec = -50.0;
    if (isnan(data.tempMinRec) || data.tempMinRec < -40.0 || data.tempMinRec == 0.0) data.tempMinRec = 100.0;
}
...
void StorageManager::save() {
    ...
    EEPROM.put(ADDR_VOLT_CAL, data.voltCalFactor);
    // Não salvamos Max/Min aqui para não desgastar à toa, eles são salvos em updateRecords
    EEPROM.commit();
}
